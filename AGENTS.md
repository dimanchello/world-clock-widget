...
| `helpers.ts` | Чистые утилитарные функции (парсинг времени, форматирование, расчёт смещений, работа со списками) |
| `notifications.ts` | Менеджер и сервис уведомлений (NotificationManager), изолированный от UI |
| `timezones.ts` | Справочник часовых поясов и вспомогательные функции локализации меток |
...
## Тесты
- Фреймворк: Vitest
- Чистые функции покрыты в `__tests__/helpers.test.ts`
- Сервис уведомлений покрыт в `__tests__/notifications.test.ts`
- Справочник поясов покрыт в `__tests__/timezones.test.ts`
- Локализация покрыта в `__tests__/i18n.test.ts`
- Тесты проверяют: корректный ввод, граничные случаи, ночные/дневные смены, ролловер дат, дедубликацию
- 77 тестов, все проходят
